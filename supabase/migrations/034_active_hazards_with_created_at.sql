-- Active hazard feed with creation timestamp for client-side time filters.
CREATE OR REPLACE FUNCTION get_active_hazards_in_radius_v2(
...
BEGIN
  ...
  SELECT h.id, h.hazard_number, h.type::TEXT, h.severity::TEXT,
    ...
    round(haversine_distance(p_latitude, p_longitude,
      ST_Y(h.location::geometry), ST_X(h.location::geometry)) / 1000, 2),
    h.created_at
...
GRANT EXECUTE ON FUNCTION get_active_hazards_in_radius_v2(DOUBLE PRECISION, DOUBLE PRECISION, NUMERIC)
  TO authenticated;
