-- PfotenNetz Migration 046: Rely on EXECUTE grants for hazard RPC auth
--
-- auth.uid() is NULL for service-role calls by design. The function is already
-- protected by PUBLIC revoke + authenticated execute grant, while service-role
-- callers are trusted server-side callers.
